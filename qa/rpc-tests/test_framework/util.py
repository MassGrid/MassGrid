...
MASSGRIDD_PROC_WAIT_TIMEOUT = 60
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "massgrid.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_massgridd_start(process, url, i):
    '''
    Wait for massgridd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if massgridd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('massgridd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("MGDD", "massgridd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            if redirect_stderr:
                stderr = sys.stdout
            massgridd_processes[i] = subprocess.Popen(args, stderr=stderr)
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: massgridd started, waiting for RPC to come up")
            wait_for_massgridd_start(massgridd_processes[i], rpc_url(i), i)
            if os.getenv("PYTHON_DEBUG", ""):
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    ...
    if binary is None:
        binary = os.getenv("MGDD", "massgridd")
    ...
    massgridd_processes[i] = subprocess.Popen(args, stderr=stderr)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: massgridd started, waiting for RPC to come up")
    url = rpc_url(i, rpchost)
    wait_for_massgridd_start(massgridd_processes[i], url, i)
    if os.getenv("PYTHON_DEBUG", ""):
...
def stop_node(node, i):
    ...
    return_code = massgridd_processes[i].wait(timeout=MASSGRIDD_PROC_WAIT_TIMEOUT)
...
def stop_nodes(nodes):
    for i, node in enumerate(nodes):
        stop_node(node, i)
    assert not massgridd_processes.values() # All connections must be gone now
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s MGD too low! (Should be %s MGD)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s MGD too high! (Should be %s MGD)"%(str(fee), str(target_fee)))
