    ...
    m_mousePress(false)
{
    ...
    QString version = tr("MassGrid") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About MassGrid"));
        ...
        std::string strUsage = HelpMessage(HMM_MASSGRID_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MassGridGUI::DEFAULT_UIPLATFORM));
        }
...
All the MassGrid in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 MGD, 0.1 MGD, 1 MGD and 10 MGD -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://massgrid.atlassian.net/wiki/display/DOC/PrivateSend\">https://massgrid.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
    }
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("MassGrid is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(MassGridGUI *window)
{
    if (!window)
        return nullptr;
