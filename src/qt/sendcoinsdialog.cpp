    ...
    platformStyle(platformStyle)
{
    ...
    connect(ui->checkboxSubtractFeeFromAmount, SIGNAL(toggled(bool)), this, SIGNAL(coinControlUpdateLabels()));
    ...
    ui->groupCustomFee->setId(ui->radioCustomPerKilobyte_, 0);
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    strFunds = tr("using") + " <b>" + tr("any available funds") + "</b>";
...
std::string SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QString strFunds,bool isSendToMasternode, std::string masternodeip)
{
    ...
    processSendCoinsReturn(prepareStatus,
        MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, currentTransaction.getRecipients())
    {
        ...
        QString amount = "<b>" + MassGridUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(MassGridUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH(MassGridUnits::Unit u, MassGridUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(MassGridUnits::formatHtmlWithUnit(u, totalAmount));
    }
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), bal));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
    case WalletModel::PaymentRequestExpired:
        msgParams.first = tr("Payment request expired.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    case WalletModel::InvalidPubkey:
        msgParams.first = tr("The transaction vin does not contain the default address's public key,please confirm the address has enough account!");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
{
    ui->radioCustomPerKilobyte_->setChecked(true);
...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    ui->radioCustomPerKilobyte_  ->setEnabled(ui->radioCustomFee->isChecked() && !ui->checkBoxMinimumFee->isChecked());
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
            ((ui->radioCustomPerKilobyte_->isChecked()) ? "/kB" : ""));
    }
}
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(MassGridUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CMassGridAddress addr = CMassGridAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid MassGrid address"));
        }
        else // Valid address
        {
            CKeyID keyid;
