...
{
    return QCoreApplication::translate("massgrid-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. massgrid_de.qm (shortcut "de" needs to be defined in massgrid.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. massgrid_de_DE.qm (shortcut "de_DE" needs to be defined in massgrid.qrc)
...
/** Class encapsulating MassGrid startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class MassGrid: public QObject
{
    Q_OBJECT
public:
    explicit MassGrid();
...
/** Main MassGrid application object */
class MassGridApplication: public QApplication
{
    Q_OBJECT
public:
    explicit MassGridApplication(int &argc, char **argv);
    ~MassGridApplication();
    ...
    /// Get window identifier of QMainWindow (MassGridGUI)
...
void MassGrid::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(strMiscWarning));
}

void MassGrid::initialize()
{
...
void MassGrid::restart(QStringList args)
{
...
MassGridApplication::MassGridApplication(int &argc, char **argv):
    ...
    returnValue(0)
{
    ...
    // This must be done inside the MassGridApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = GetArg("-uiplatform", MassGridGUI::DEFAULT_UIPLATFORM);
...
MassGridApplication::~MassGridApplication()
{
    if(coreThread)
    {
...
#ifdef ENABLE_WALLET
void MassGridApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

void MassGridApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new MassGridGUI(platformStyle, networkStyle, 0);
...
void MassGridApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void MassGridApplication::startThread()
{
    if(coreThread)
        return;
    coreThread = new QThread(this);
    MassGrid *executor = new MassGrid();
...
void MassGridApplication::parameterSetup()
{
    InitLogging();
    InitParameterInteraction();
}

void MassGridApplication::requestInitialize()
{
...
void MassGridApplication::requestShutdown()
{
...
void MassGridApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            ...
            window->addWallet(MassGridGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MassGridGUI::DEFAULT_WALLET);
        ...
        // massgrid: URIs or payment requests:
...
void MassGridApplication::shutdownResult(int retval)
{
...
WId MassGridApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(massgrid);
    Q_INIT_RESOURCE(massgrid_locale);

    MassGridApplication app(argc, argv);
    ...
    try {
        ...
        CMessageBox::critical(0, QObject::tr("MassGrid"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    // massgrid: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MassGrid didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
