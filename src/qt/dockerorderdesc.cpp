...
QString DockerOrderDesc::FormatTxStatus(const CWalletTx& wtx)
{
...
QString DockerOrderDesc::toHTML(CWallet *wallet, CWalletTx &wtx, DockerOrderRecord *rec, int unit)
{
    QString strHTML;
    ...
    strHTML += "<b>" + tr("Transaction ID") + ":</b> " + DockerOrderRecord::formatSubTxId(wtx.GetHash(), rec->idx) + "<br>";
    strHTML += "<b>" + tr("Transaction total size") + ":</b> " + QString::number(wtx.GetTotalSize()) + " bytes<br>";

    // Message from normal massgrid:URI (massgrid:XyZ...?message=example)
