...
/** 
 * Fees smaller than this (in nMGD) are considered zero fee (for transaction creation)
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        BOOST_FOREACH(CompactTallyItem& item, vecTallyRet)
            strMessage += strprintf("  %s %f\n", CMassGridAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 MGD."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                std::vector<CInputCoin> vecTxInTmp;
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins){
                    CTxIn txin = CTxIn(coin.first->GetHash(),coin.second,CScript(),
                                              std::numeric_limits<unsigned int>::max()-1);
                    vecTxInTmp.push_back(CInputCoin(txin, coin.first->vout[coin.second].scriptPubKey));
                    txNew.vin.push_back(txin);
                }

                sort(txNew.vin.begin(), txNew.vin.end(), CompareInputBIP69());
                sort(vecTxInTmp.begin(), vecTxInTmp.end(), CompareInputBIP69());
                ...
                for (const auto& txin : vecTxInTmp)
                {
                    bool signSuccess;
