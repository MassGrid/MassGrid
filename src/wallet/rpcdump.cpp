...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"massgridprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"massgridprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CMassGridAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CMassGridAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MassGrid address or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CMassGridAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const UniValue& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CMassGridAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"massgridaddress\"\n"
            "\nReveals the private key corresponding to 'massgridaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"massgridaddress\"   (string, required) The massgrid address for the private key\n"
            "\nResult:\n"
    ...
    return CMassGridSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by MassGrid %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (pwalletMain->GetHDChain(hdChainCurrent))
    {
        ...
        CMassGridExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CMassGridAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CMassGridSecret(key).ToString(), strTime);
