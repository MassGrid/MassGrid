...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        if(pindexPrev->nHeight<84999 && Params().NetworkIDString()=="main")
                pblock->nVersion=5;
        ...
        // NOTE: unlike in massgrid, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in MassGrid
//
...
// ***TODO*** that part changed in massgrid, we are using a mix with old one here for now
void static MassGridMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("MassGridMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
...
void GenerateMassGrids(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MassGridMiner, boost::cref(chainparams), boost::ref(connman)));
}
void StopMiner()
{
