...
{
    if(fLiteMode) return; // disable all MassGrid specific functionality
...
bool CInstantSend::getTxLockCandidate(std::map<uint256, CTxLockCandidate> &TxLockCandidate)
{
    LOCK(cs_instantsend);
    if(mapTxLockCandidates.empty())
        return false;
    TxLockCandidate=mapTxLockCandidates;
    return true;
}
...
{
    //LogPrintf("IsLockedInstantSendTransaction: 0 %d,%d,%d,%d\n",!fEnableInstantSend,fLargeWorkForkFound,fLargeWorkInvalidChainFound,!sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING));
    if(!fEnableInstantSend || fLargeWorkForkFound || fLargeWorkInvalidChainFound ||
...
std::string CInstantSend::ToString()
{
    LOCK(cs_instantsend);
    // LogPrintf("****\n");
    // std::map<uint256, CTxLockCandidate>::iterator iter;//定义一个迭代指针iter
    // for(iter=mapTxLockCandidates.begin(); iter!=mapTxLockCandidates.end(); ++iter){
    //     LogPrintf("txid: %s, CTxLockCandidate:\n",(iter->first).ToString());
    // }
    // LogPrintf("****\n");
