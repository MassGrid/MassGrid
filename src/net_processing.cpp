...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    ...
    //     printf("nChainWork %f   nMinimumChainWork %f result %s\n",(state->pindexBestKnownBlock->nChainWork).getdouble(),UintToArith256(consensusParams.nMinimumChainWork).getdouble(),state->pindexBestKnownBlock->nChainWork < UintToArith256(consensusParams.nMinimumChainWork)?"TRUE":"FALSE");
    //     printf("pindexBestKnownBlock->nChainWork  %s >= consensusParams.nMinimumChainWork %s result %s height %d\n",state->pindexBestKnownBlock->nChainWork.ToString().c_str(),UintToArith256(consensusParams.nMinimumChainWork).ToString().c_str(),state->pindexBestKnownBlock->nChainWork < UintToArith256(consensusParams.nMinimumChainWork)?"TRUE":"FALSE",state->pindexBestKnownBlock->nHeight);
    // }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        MassGrid Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else
    {
        ...
        if (found)
        {
            ...
            dockerServerman.ProcessMessage(pfrom, strCommand, vRecv, connman);
...
bool SendMessages(CNode* pto, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    const Consensus::Params& consensusParams = chainParams.GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in MassGrid
        ...
        if (state.fSyncStarted && state.nHeadersSyncTimeout < std::numeric_limits<int64_t>::max()) {
            ...
            if (pindexBestHeader->GetBlockTime() <= GetAdjustedTime() - 6*60*60) { // was 24*60*60 in massgrid
