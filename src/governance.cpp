    ...
    while(it != mapObjects.end())
    {
        ...
        if((pObj->IsSetCachedDelete() || pObj->IsSetExpired()) &&
           (nTimeSinceDeletion >= GOVERNANCE_DELETION_DELAY)) {
            ...
            int64_t nMasternodePaymentsIncreaseBlockSeconds = Params().GetConsensus().nMasternodePaymentsIncreaseBlock * Params().GetConsensus().nPowTargetSpacing;
            int64_t nTimeExpired = pObj->GetCreationTime() + 2 * nMasternodePaymentsIncreaseBlockSeconds + GOVERNANCE_DELETION_DELAY;
...
bool CGovernanceManager::MasternodeRateCheck(const CGovernanceObject& govobj, bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    ...
    if(nTimestamp < nNow - 2 * nMasternodePaymentsIncreaseBlockSeconds) {
    ...
    double dMaxRate = 1.1 / nMasternodePaymentsIncreaseBlockSeconds;
    double dRate = 0.0;
    ...
    case GOVERNANCE_OBJECT_TRIGGER:
        ...
        dMaxRate = 2 * 1.1 / double(nMasternodePaymentsIncreaseBlockSeconds);
        break;
    case GOVERNANCE_OBJECT_WATCHDOG:
...
void CGovernanceManager::CheckPostponedObjects(CConnman& connman)
{
    ...
    for(hash_s_it it = setAdditionalRelayObjects.begin(); it != setAdditionalRelayObjects.end();) {
        ...
        if(itObject != mapObjects.end()) {
            ...
            bool fValid = (nTimestamp <= nNow + MAX_TIME_FUTURE_DEVIATION) && (nTimestamp >= nNow - 2 * nMasternodePaymentsIncreaseBlockSeconds);
...
void CGovernanceManager::RequestGovernanceObject(CNode* pfrom, const uint256& nHash, CConnman& connman, bool fUseFilter)
{
    ...
    if(fUseFilter) {
        ...
        if(pObj) {
            filter = CBloomFilter(Params().GetConsensus().nGovernanceFilterElements, GOVERNANCE_FILTER_FP_RATE, GetRandInt(944299), BLOOM_UPDATE_ALL);
