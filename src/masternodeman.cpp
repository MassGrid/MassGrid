...
bool CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet)
{
    ...
    if (!masternodeSync.IsWinnersListSynced()) {
        LogPrintf("CMasternodeMan: without winner list we can't reliably find the next winner anyway %d.\n",masternodeSync.IsWinnersListSynced());
        return false;
    }
    ...
    */
    ...
    LogPrintf("CMasternodeMan:  CountMasternodes = %d.\n",nCountRet);
    ...
    LogPrintf("CMasternodeMan:  mnInfoRet.fInfoValid = %d, pBestMasternode = %d.\n",mnInfoRet.fInfoValid,pBestMasternode);
...
bool CMasternodeMan::GetMasternodeScores(const uint256& nBlockHash, CMasternodeMan::score_pair_vec_t& vecMasternodeScoresRet, int nMinProtocol)
{
    ...
    // calculate scores
    for (auto& mnpair : mapMasternodes) {
        //LogPrintf("mnpair.second.nProtocolVersion: %d\n",mnpair.second.nProtocolVersion);
...
// Verification of masternodes via unique direct requests. //privatesend
...
bool CMasternodeMan::SendVerifyRequest(const CAddress& addr, const std::vector<CMasternode*>& vSortedByAddr, CConnman& connman)
{
    ...
    CMasternodeVerification mnv(addr, GetRandInt(944299), nCachedBlockHeight - 1);
