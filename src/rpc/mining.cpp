...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(MASSGRID_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    GenerateMassGrids(fGenerate, nGenProcLimit, Params(), *g_connman);
...
// NOTE: Unlike wallet RPC (which use MGD values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_nMGD / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in nMGD) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/massgrid/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/massgrid/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in nMGD); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in nMGD)\n"
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MassGrid is downloading masternode winners...");
    ...
    if(pblock->txoutMasternode != CTxOut()) {
        ...
        CMassGridAddress address2(address1);
...
UniValue estimatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in MGD)\n"
