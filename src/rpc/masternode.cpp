...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print MassGrid address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CMassGridAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                               ...
                               mn.lastPing.mdocker.protocolVersion<< " " << 
                ...
                CMassGridAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        BOOST_FOREACH(CMasternodeBroadcast& mnb, vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CMassGridAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CMassGridAddress(mnb.pubKeyMasternode.GetID()).ToString()));
                ...
                dockerObj.push_back(Pair("protocolVersion",mnb.lastPing.mdocker.protocolVersion));
