...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of massgrid addresses\n"
...
UniValue getblockchaininfo(const UniValue& params, bool fHelp)
{
    ...
    softforks.push_back(SoftForkDesc("mip10", 5, tip, consensusParams));
...
UniValue getchaintxstats(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
            throw runtime_error(
                ...
                "\nCompute statistics about the total number and rate of transactions in the chain.\n"
                    "nblocks, 3, /* default */ one month Size of the window in number of blocks"
                    "blockhash, 7, /* default */ chain tip , The hash of the block that ends the window."
                "\nResult:\n"
                    "{\n"
    ...
    if (params[1].isNull()) {
        ...
        pindex = mi != mapBlockIndex.end()?mi->second:nullptr;
        if (!pindex) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Block not found");
        }
