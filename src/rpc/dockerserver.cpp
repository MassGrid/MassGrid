...
UniValue docker(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw std::runtime_error(
            ...
            "       5. \"SSH_PUBKEY\"(string, required)\n"
            "       6. \"CPU name\"(string, required)\n"
            "       7. \"CPU thread count\"(int, required)\n"
            "       8. \"Memory name\"(string, required)\n"
            "       9. \"Memory GByte\"(int, required)\n"
            "       10. \"GPU Kind\"(string, required)\n"
            "       11. \"GPU count\"(int, required)\n"
            ...
            "       3. \"COutPoint the update outpoint hash (string, required)\n"
    ...
    if (strCommand == "create") {
        ...
        boost::split(fields, strOutpoint, boost::is_any_of("-"), boost::token_compress_on);
        if (fields.size() < 2) {
            return "parameter error" + strOutpoint;
        }
        dockerCreateService.clusterServiceCreate.OutPoint = COutPoint(uint256S(fields[0]), boost::lexical_cast<int>(fields[1]));
        ...
        std::string strssh_pubkey = params[5].get_str();
        dockerCreateService.clusterServiceCreate.SSHPubkey = strssh_pubkey;

        dockerCreateService.clusterServiceCreate.hardware.CPUType = params[6].get_str();
        int64_t serviceCpu = params[7].get_int64();
        dockerCreateService.clusterServiceCreate.hardware.CPUThread = serviceCpu;

        dockerCreateService.clusterServiceCreate.hardware.MemoryType = params[8].get_str();
        int64_t serviceMemoey_byte = params[9].get_int64();
        dockerCreateService.clusterServiceCreate.hardware.MemoryCount = serviceMemoey_byte;

        std::string strServiceGpuName = params[10].get_str();
        dockerCreateService.clusterServiceCreate.hardware.GPUType = strServiceGpuName;

        int64_t serviceGpu = params[11].get_int64();
        dockerCreateService.clusterServiceCreate.hardware.GPUCount = serviceGpu;

        dockerCreateService.clusterServiceCreate.hardware.PersistentStore = params[12].get_str();
    ...
    if (strCommand == "update") {
        ...
        std::string strOutpoint2 = params[3].get_str();
        std::vector<std::string> fields2;
        boost::split(fields2, strOutpoint2, boost::is_any_of("-"), boost::token_compress_on);
        if (fields2.size() < 2) {
            return "parameter error" + strOutpoint2;
        }
        updateService.clusterServiceUpdate.CrerateOutPoint = COutPoint(uint256S(fields[0]), boost::lexical_cast<int>(fields[1]));
        updateService.clusterServiceUpdate.OutPoint = COutPoint(uint256S(fields2[0]), boost::lexical_cast<int>(fields2[1]));
        EnsureWalletIsUnlocked();
    ...
    if (strCommand == "delete") {
        ...
        delService.CrerateOutPoint = COutPoint(uint256S(fields[0]), boost::lexical_cast<int>(fields[1]));
        EnsureWalletIsUnlocked();
    ...
    if (strCommand == "listprice") {
        ...
        for (auto it = entries.begin(); it != entries.end(); ++it) {
            ...
            streamFull << it->getType() << " " << it->getName() << " " << (double)it->getPrice() / COIN;
    ...
    if (strCommand == "getservice") {
        ...
        COutPoint outpoint(uint256S(fields[0]), boost::lexical_cast<int>(fields[1]));
    ...
    if (strCommand == "getservices") {
        ...
        int64_t count =boost::lexical_cast<int64_t>(params[3].get_str());
