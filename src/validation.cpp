                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s MGD additional fees, %d delta bytes\n",
...
bool CheckTransactionInputScriptPubkey(const uint256 &hash, CTransaction &txOut,const CPubKey& pubKey, const Consensus::Params& consensusParams, uint256 &hashBlock,std::string& strErr, bool fAllowSlow){
    ...
    strErr = "GetTransaction Error txid: "+hash.ToString();
    return false;
}
...
/*
NOTE:   unlike massgrid we are using PREVIOUS block height here,
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if(Params().NetworkIDString()=="main"){
        ...
        bool fChecksTMIP20 =pindex->nHeight>=1500 && block.nVersion==6;
        if(!fChecksTMIP20){
            if(pindex->nHeight>=1500)
                return state.DoS(100, error("ConnectBlock() : tried to overwrite transaction"),
    ...
    bool fEnforceBIP30 =true;// fEnforceBIP30 && (!pindexBIP34height || !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    // MGD : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(MGD): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(MGD): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MGD : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(MGD): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(MGD): spork is off, skipping transaction locking checks\n");
    }
