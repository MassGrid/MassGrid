...
const char * const MASSGRID_CONF_FILENAME = "massgrid.conf";
const char * const MASSGRID_PID_FILENAME = "massgridd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "massgrid" is a composite category enabling all MassGrid-related debug output
            if(ptrCategory->count(string("massgrid"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MassGrid
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MassGrid
    // Mac: ~/Library/Application Support/MassGrid
    // Unix: ~/.massgrid
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", MASSGRID_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty massgrid.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override massgrid.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", MASSGRID_PID_FILENAME));
