...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    if(nBlockHeight < Params().GetConsensus().nMasternodePaymentsStartBlock){
        LogPrintf("nBlockHeight %d nMasternodePaymentsStartBlock %d MasternodePayment: FALSE\n",nBlockHeight,Params().GetConsensus().nMasternodePaymentsStartBlock);
        return;
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f MGD\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
