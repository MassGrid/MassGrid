MassGrid version 0.10.0 is now available from:
...
backwards-compatible with older versions of MassGrid or other software:
...
MassGrid now uses 'headers-first synchronization'. This means that we first
...
Starting from 0.10.0, the MassGrid distribution includes a consensus library.
...
This library is called `libmassgridconsensus.so` (or, `.dll` for Windows).
...
standard MassGrid nodes wouldn't relay them to miners, nor would
...
MassGrid's block templates are now for version 3 blocks only, and any mining
...
MassGrid now supports BIP 22 long polling, so mining software can be
notified immediately of new templates rather than having to poll periodically.

Support for BIP 23 block proposals is now available in MassGrid's
...
MassGrid 0.10 implements BIP 66, which introduces block version 3, and a new
...
- `516053c` Make links in 'About MassGrid' clickable
