...
MassGrid 0.11.0 is forked off the MassGrid 0.9.3. The old MassGrid
...
notes of MassGrid  0.9.0, 0.9.1, 0.9.2, 0.9.2.1, 0.9.3 and was completed
with the introduced changes to the MassGrid 0.11.0.
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`DRKP`/`DRKV`
  for testnet)
- Adapted BIP44 coin type `5` for MassGrid (0x80000005) as defined in SLIP-0044
- Added new units: `nMGD` (1 / 100.000.000 DRK)
- Added units for testnet: tDRK, mtDRK, utDRK, tnMGD
...
- Fix "Start MassGrid on system login"
