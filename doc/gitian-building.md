...
*Setup instructions for a Gitian build of MassGrid using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MassGrid
...
- [Building MassGrid](#building-massgrid-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-massgrid-host/README.md).
...
Clone the git repositories for MassGrid and Gitian.
...
in the MassGrid repository under 'Fetch and build inputs' to install sources which require
...
To build MassGrid (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MassGrid repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/massgrid/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/massgrid/massgrid
...
For example:
```bash
URL=https://github.com/crowning-/massgrid.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit massgrid=${COMMIT} --url massgrid=${URL} ../massgrid/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit massgrid=${COMMIT} --url massgrid=${URL} ../massgrid/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit massgrid=${COMMIT} --url massgrid=${URL} ../massgrid/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the massgrid git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../massgrid/contrib/gitian-descriptors/*|sort|uniq )
...
MGDPATH=/some/root/path/massgrid.git
SIGPATH=/some/root/path/massgrid-detached-sigs.git

./bin/gbuild --url massgrid=${MGDPATH},signature=${SIGPATH} ../massgrid/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/massgrid-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/massgrid-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/massgrid-osx-build.assert
```
...
[massgrid/gitian.sigs](https://github.com/massgrid/gitian.sigs/) repository, or if that's not possible to create a pull
request.
