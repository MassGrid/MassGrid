...
memory available when compiling MassGrid. On systems with less, gcc can be
...
**For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~massgrid/+archive/massgrid).
...
See the section "Disable-wallet mode" to build MassGrid without wallet.
...
# Configure MassGrid to use our own-built instance of BDB
cd $MGD_ROOT
./autogen.sh
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/massgrid/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard MassGrid distributions and independently built
...
with 4.8-built MassGrid is needed follow the steps under "Berkeley DB" above.
